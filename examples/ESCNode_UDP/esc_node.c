/*
  ...
  This example uses multicast UDP for CAN transport

  Example usage: ./esc_node mcast:
*/
/*
...
*/
static void processTxRxOnce(MCASTCANInstance *mcast, int32_t timeout_msec)
{
    // Transmitting
    for (const CanardCANFrame* txf = NULL; (txf = canardPeekTxQueue(&canard)) != NULL;) {
        const int16_t tx_res = mcastTransmit(mcast, txf, 0);
    ...
    const int16_t rx_res = mcastReceive(mcast, &rx_frame, timeout_msec);
    if (rx_res < 0) {
...
int main(int argc, char** argv)
{
    ...
    /*
     * Initializing the CAN backend driver; in this example we're using Mcast
     */
    MCASTCANInstance mcast;
    const char* const can_iface_name = argv[1];
    int16_t res = mcastInit(&mcast, can_iface_name);
    if (res < 0) {
